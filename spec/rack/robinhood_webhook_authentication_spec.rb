...
describe Rack::RobinhoodWebhookAuthentication do
  before do
  ...
  describe 'new' do
    it 'should initialize with an app, auth token and a path' do
      expect {
        Rack::RobinhoodWebhookAuthentication.new(@app, 'ABC', /\/voice/)
      }.not_to raise_error
    end

    it 'should initialize with an app, auth token and paths' do
      expect {
        Rack::RobinhoodWebhookAuthentication.new(@app, 'ABC', /\/voice/, /\/sms/)
      }.not_to raise_error
    end

    it 'should initialize with an app, dynamic token and paths' do
      expect {
        Rack::RobinhoodWebhookAuthentication.new(@app, nil, /\/voice/, /\/sms/)
      }.not_to raise_error
    end
  end
  ...
  describe 'calling against one path with dynamic auth token' do
    it 'should allow a request through if it validates' do
      ...
      expect_any_instance_of(Rack::Request).to receive(:POST).and_return({'username' => account_sid})
      @middleware = Rack::RobinhoodWebhookAuthentication.new(@app, nil, /\/voice/) { |asid| auth_token}
      request_validator = double('RequestValidator')
      expect(Robinhood::Util::RequestValidator).to receive(:new).with(auth_token).and_return(request_validator)
  ...
  describe 'calling against one path' do
    before do
      @middleware = Rack::RobinhoodWebhookAuthentication.new(
        @app, 'ABC', /\/voice/
      )
    end

    it 'should not intercept when the path doesn\'t match' do
      expect(Robinhood::Util::RequestValidator).to_not receive(:validate)
    ...
    it 'should allow a request through if it validates' do
      expect_any_instance_of(Robinhood::Util::RequestValidator).to(
